\hypertarget{md_googletest_docs_primer_autotoc_md222}{}\doxysection{Introduction\+: Why Google\+Test?}\label{md_googletest_docs_primer_autotoc_md222}
...
\item \mbox{\hyperlink{structTests}{Tests}} should be {\itshape fast}. With Google\+Test, you can reuse shared resources across tests and pay for the set-\/up/tear-\/down only once, without making tests depend on each other.
\end{DoxyEnumerate}

Since Google\+Test is based on the popular x\+Unit architecture, you\textquotesingle{}ll feel right at home if you\textquotesingle{}ve used JUnit or Py\+Unit before. If not, it will take you about 10 minutes to learn the basics and get started. So let\textquotesingle{}s go!\hypertarget{md_googletest_docs_primer_autotoc_md223}{}\doxysection{Beware of the Nomenclature}\label{md_googletest_docs_primer_autotoc_md223}
...
\cellcolor{\tableheadbgcolor}\textbf{ Meaning   }&\cellcolor{\tableheadbgcolor}\textbf{ Google\+Test Term   }&\cellcolor{\tableheadbgcolor}\textbf{ \href{https://www.istqb.org/}{\texttt{ ISTQB}} Term    }\\\cline{1-3}
\endhead
Exercise a particular program path with specific input values and verify the results   &\href{\#simple-tests}{\texttt{ TEST()}}   &\href{https://glossary.istqb.org/en/search/test\%20case}{\texttt{ Test Case}}   \\\cline{1-3}
\end{longtabu}
\hypertarget{md_googletest_docs_primer_autotoc_md224}{}\doxysection{Basic Concepts}\label{md_googletest_docs_primer_autotoc_md224}
...
{\itshape \mbox{\hyperlink{structTests}{Tests}}} use assertions to verify the tested code\textquotesingle{}s behavior. If a test crashes or has a failed assertion, then it {\itshape fails}; otherwise it {\itshape succeeds}.
...
We\textquotesingle{}ll now explain how to write a test program, starting at the individual assertion level and building up to tests and test suites.\hypertarget{md_googletest_docs_primer_autotoc_md225}{}\doxysection{Assertions}\label{md_googletest_docs_primer_autotoc_md225}
...
Google\+Test provides a collection of assertions for verifying the behavior of your code in various ways. You can check Boolean conditions, compare values based on relational operators, verify string values, floating-\/point values, and much more. There are even assertions that enable you to verify more complex states by providing custom predicates. For the complete list of assertions provided by Google\+Test, see the \mbox{\hyperlink{md_googletest_docs_reference_assertions}{Assertions Reference}}.\hypertarget{md_googletest_docs_primer_autotoc_md226}{}\doxysection{Simple Tests}\label{md_googletest_docs_primer_autotoc_md226}
To create a test\+:
...
{\bfseries{Availability}}\+: Linux, Windows, Mac.\hypertarget{md_googletest_docs_primer_same-data-multiple-tests}{}\doxysection{Test Fixtures\+: Using the Same Data Configuration for Multiple Tests}\label{md_googletest_docs_primer_same-data-multiple-tests}
...
Unlike {\ttfamily TEST()}, in {\ttfamily TEST\+\_\+\+F()} the first argument must be the name of the test fixture class. ({\ttfamily \+\_\+F} stands for \char`\"{}\+Fixture\char`\"{}). No test suite name is specified for this macro.
...
{\bfseries{Availability}}\+: Linux, Windows, Mac.\hypertarget{md_googletest_docs_primer_autotoc_md227}{}\doxysection{Invoking the Tests}\label{md_googletest_docs_primer_autotoc_md227}
...
{\bfseries{Availability}}\+: Linux, Windows, Mac.\hypertarget{md_googletest_docs_primer_autotoc_md228}{}\doxysection{Writing the main() Function}\label{md_googletest_docs_primer_autotoc_md228}
...
The {\ttfamily testing\+::\+Init\+Google\+Test()} function parses the command line for Google\+Test flags, and removes all recognized flags. This allows the user to control a test program\textquotesingle{}s behavior via various flags, which we\textquotesingle{}ll cover in the \mbox{\hyperlink{md_googletest_docs_advanced}{Advanced\+Guide}}. You {\bfseries{must}} call this function before calling {\ttfamily RUN\+\_\+\+ALL\+\_\+\+TESTS()}, or the flags won\textquotesingle{}t be properly initialized.
...
\{\+: .callout .note\} NOTE\+: {\ttfamily Parse\+GUnit\+Flags()} is deprecated in favor of {\ttfamily Init\+Google\+Test()}.\hypertarget{md_googletest_docs_primer_autotoc_md229}{}\doxysection{Known Limitations}\label{md_googletest_docs_primer_autotoc_md229}

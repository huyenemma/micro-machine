\hypertarget{md_googletest_docs_gmock_for_dummies_autotoc_md200}{}\doxysection{What Is g\+Mock?}\label{md_googletest_docs_gmock_for_dummies_autotoc_md200}
...
\end{DoxyEnumerate}\hypertarget{md_googletest_docs_gmock_for_dummies_autotoc_md201}{}\doxysection{Why g\+Mock?}\label{md_googletest_docs_gmock_for_dummies_autotoc_md201}
...
\end{DoxyItemize}\hypertarget{md_googletest_docs_gmock_for_dummies_autotoc_md202}{}\doxysection{Getting Started}\label{md_googletest_docs_gmock_for_dummies_autotoc_md202}
g\+Mock is bundled with googletest.\hypertarget{md_googletest_docs_gmock_for_dummies_autotoc_md203}{}\doxysection{A Case for Mock Turtles}\label{md_googletest_docs_gmock_for_dummies_autotoc_md203}
...
Your program will normally use a real implementation of this interface. In tests, you can use a mock implementation instead. This allows you to easily check what drawing primitives your program is calling, with what arguments, and in which order. \mbox{\hyperlink{structTests}{Tests}} written this way are much more robust (they won\textquotesingle{}t break because your new machine does anti-\/aliasing differently), easier to read and maintain (the intent of a test is expressed in the code, not in some binary images), and run {\itshape much, much faster}.\hypertarget{md_googletest_docs_gmock_for_dummies_autotoc_md204}{}\doxysection{Writing the Mock Class}\label{md_googletest_docs_gmock_for_dummies_autotoc_md204}
If you are lucky, the mocks you need to use have already been implemented by some nice people. If, however, you find yourself in the position to write a mock class, relax -\/ g\+Mock turns this task into a fun game! (Well, almost.)\hypertarget{md_googletest_docs_gmock_for_dummies_autotoc_md205}{}\doxysubsection{How to Define It}\label{md_googletest_docs_gmock_for_dummies_autotoc_md205}
...
You don\textquotesingle{}t need to define these mock methods somewhere else -\/ the {\ttfamily MOCK\+\_\+\+METHOD} macro will generate the definitions for you. It\textquotesingle{}s that simple!\hypertarget{md_googletest_docs_gmock_for_dummies_autotoc_md206}{}\doxysubsection{Where to Put It}\label{md_googletest_docs_gmock_for_dummies_autotoc_md206}
...
Admittedly, this test is contrived and doesn\textquotesingle{}t do much. You can easily achieve the same effect without using g\+Mock. However, as we shall reveal soon, g\+Mock allows you to do {\itshape so much more} with the mocks.\hypertarget{md_googletest_docs_gmock_for_dummies_autotoc_md208}{}\doxysection{Setting Expectations}\label{md_googletest_docs_gmock_for_dummies_autotoc_md208}
The key to using a mock object successfully is to set the {\itshape right expectations} on it. If you set the expectations too strict, your test will fail as the result of unrelated changes. If you set them too loose, bugs can slip through. You want to do it just right such that your test can catch exactly the kind of bugs you intend it to catch. g\+Mock provides the necessary means for you to do it \char`\"{}just right.\char`\"{}\hypertarget{md_googletest_docs_gmock_for_dummies_autotoc_md209}{}\doxysubsection{General Syntax}\label{md_googletest_docs_gmock_for_dummies_autotoc_md209}
...
{\ttfamily \+\_\+} is an instance of what we call {\bfseries{matchers}}. A matcher is like a predicate and can test whether an argument is what we\textquotesingle{}d expect. You can use a matcher inside {\ttfamily EXPECT\+\_\+\+CALL()} wherever a function argument is expected. {\ttfamily \+\_\+} is a convenient way of saying \char`\"{}any value\char`\"{}.

In the above examples, {\ttfamily 100} and {\ttfamily 50} are also matchers; implicitly, they are the same as {\ttfamily Eq(100)} and {\ttfamily Eq(50)}, which specify that the argument must be equal (using {\ttfamily operator==}) to the matcher argument. There are many \mbox{\hyperlink{md_googletest_docs_reference_matchers}{built-\/in matchers}} for common types (as well as \href{gmock_cook_book.md\#NewMatchers}{\texttt{ custom matchers}}); for example\+:
...
This works for all non-\/overloaded methods; if a method is overloaded, you need to help g\+Mock resolve which overload is expected by specifying the number of arguments and possibly also the \href{gmock_cook_book.md\#SelectOverload}{\texttt{ types of the arguments}}.\hypertarget{md_googletest_docs_gmock_for_dummies_autotoc_md211}{}\doxysubsection{Cardinalities\+: How Many Times Will It Be Called?}\label{md_googletest_docs_gmock_for_dummies_autotoc_md211}
...
{\bfseries{Quick quiz\+:}} what do you think will happen if a function is expected to be called twice but actually called four times?\hypertarget{md_googletest_docs_gmock_for_dummies_autotoc_md212}{}\doxysubsection{Actions\+: What Should It Do?}\label{md_googletest_docs_gmock_for_dummies_autotoc_md212}
...
What can we do inside {\ttfamily Will\+Once()} besides {\ttfamily Return()}? You can return a reference using {\ttfamily Return\+Ref(}$\ast${\ttfamily variable}$\ast${\ttfamily )}, or invoke a pre-\/defined function, among \href{gmock_cook_book.md\#using-actions}{\texttt{ others}}.
...
Instead of returning 100, 101, 102, ..., consecutively, this mock function will always return 100 as {\ttfamily n++} is only evaluated once. Similarly, {\ttfamily Return(new Foo)} will create a new {\ttfamily Foo} object when the {\ttfamily EXPECT\+\_\+\+CALL()} is executed, and will return the same pointer every time. If you want the side effect to happen every time, you need to define a custom action, which we\textquotesingle{}ll teach in the \mbox{\hyperlink{md_googletest_docs_gmock_cook_book}{cook book}}.
...
(What if you care about the relative order of some of the calls, but not all of them? Can you specify an arbitrary partial order? The answer is ... yes! The details can be found \href{gmock_cook_book.md\#OrderedCalls}{\texttt{ here}}.)\hypertarget{md_googletest_docs_gmock_for_dummies_StickyExpectations}{}\doxysubsection{All Expectations Are Sticky (\+Unless Said Otherwise)}\label{md_googletest_docs_gmock_for_dummies_StickyExpectations}

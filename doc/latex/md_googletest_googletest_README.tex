\hypertarget{md_googletest_googletest_README_autotoc_md391}{}\doxysubsubsection{Setup}\label{md_googletest_googletest_README_autotoc_md391}
To build Google\+Test and your tests that use it, you need to tell your build system where to find its headers and source files. The exact way to do it depends on which build system you use, and is usually straightforward.\hypertarget{md_googletest_googletest_README_autotoc_md392}{}\doxysubsection{Build with CMake}\label{md_googletest_googletest_README_autotoc_md392}
...
CMake works by generating native makefiles or build projects that can be used in the compiler environment of your choice. You can either build Google\+Test as a standalone project or it can be incorporated into an existing CMake build for another project.\hypertarget{md_googletest_googletest_README_autotoc_md393}{}\doxysubsubsection{Standalone CMake Project}\label{md_googletest_googletest_README_autotoc_md393}
...
On Mac OS X with Xcode installed, a {\ttfamily .xcodeproj} file will be generated.\hypertarget{md_googletest_googletest_README_autotoc_md394}{}\doxysubsubsection{Incorporating Into An Existing CMake Project}\label{md_googletest_googletest_README_autotoc_md394}
...
Note that this approach requires CMake 3.\+14 or later due to its use of the {\ttfamily Fetch\+Content\+\_\+\+Make\+Available()} command.\hypertarget{md_googletest_googletest_README_autotoc_md395}{}\doxyparagraph{Visual Studio Dynamic vs Static Runtimes}\label{md_googletest_googletest_README_autotoc_md395}
...
Enabling this option will make gtest link the runtimes dynamically too, and match the project in which it is included.\hypertarget{md_googletest_googletest_README_autotoc_md396}{}\doxysubsubsection{C++ Standard Version}\label{md_googletest_googletest_README_autotoc_md396}
An environment that supports C++14 is required in order to successfully build Google\+Test. One way to ensure this is to specify the standard in the top-\/level project, for example by using the {\ttfamily set(\+CMAKE\+\_\+\+CXX\+\_\+\+STANDARD 14)} command along with {\ttfamily set(\+CMAKE\+\_\+\+CXX\+\_\+\+STANDARD\+\_\+\+REQUIRED ON)}. If this is not feasible, for example in a C project using Google\+Test for validation, then it can be specified by adding it to the options for cmake via the{\ttfamily -\/DCMAKE\+\_\+\+CXX\+\_\+\+FLAGS} option.\hypertarget{md_googletest_googletest_README_autotoc_md397}{}\doxysubsection{Tweaking Google\+Test}\label{md_googletest_googletest_README_autotoc_md397}
...
We list the most frequently used macros below. For a complete list, see file \href{https://github.com/google/googletest/blob/main/googletest/include/gtest/internal/gtest-port.h}{\texttt{ include/gtest/internal/gtest-\/port.\+h}}.\hypertarget{md_googletest_googletest_README_autotoc_md398}{}\doxysubsection{Multi-\/threaded Tests}\label{md_googletest_googletest_README_autotoc_md398}
Google\+Test is thread-\/safe where the pthread library is available. After {\ttfamily \#include $<$\mbox{\hyperlink{gtest_8h_source}{gtest/gtest.\+h}}$>$}, you can check the {\ttfamily GTEST\+\_\+\+IS\+\_\+\+THREADSAFE} macro to see whether this is the case (yes if the macro is {\ttfamily \#defined} to 1, no if it\textquotesingle{}s undefined.).

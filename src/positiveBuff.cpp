...
//MaxSpeed
MaxSpeed::MaxSpeed(int duration,float boost) : Buff(duration,BuffType::Positive,BuffEffect::continuous), boost_(boost) {}

void MaxSpeed::ApplyEffect(Vehicle* vehicle){
    vehicle->ApplyBuff(1, boost_, 1, 1);
}

void MaxSpeed::ReverseEffect(Vehicle* vehicle){ vehicle->ApplyBuff(1, 1/boost_, 1, 1);}

MaxSpeed::~MaxSpeed(){}

//Magnetic Pull
Magnetic::Magnetic(int duration,float radius) : Buff(duration,BuffType::Positive,BuffEffect::continuous), radius_(radius) {}

void Magnetic::ApplyEffect(Vehicle* vehicle){
    vehicle->MagneticPull(radius_);
}

void Magnetic::ReverseEffect(Vehicle* vehicle){ vehicle->ReverseMagneticPull();}

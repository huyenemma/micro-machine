...
void Ox::SuperSkill() {
  ...
  if (superSkillCoolDown == 0) {
    // Create an instance of OxSuperSkillBuff with the specified duration and
    // intensity
    OxSuperSkillBuff* buff =
        new OxSuperSkillBuff(OX_SKILL_DURATION, OX_SKILL_INTENSITY);
    this->AddBuff(buff);

    // Set the cooldown for the next use
    superSkillCoolDown = OX_SKILL_COOLDOWN;
  } else {
    // Display a message or take some action indicating the cooldown is still
    // active

...
void Vehicle::UpdateSpeed() {
  ...
  if (std::abs(vel.Length()) > maxSpeed * MaxSpeedBuff) {
    forceMagnitude -=
        m_body->GetMass() * std::abs(vel.Length() - maxSpeed * MaxSpeedBuff);
  }
...
void Vehicle::Rotate(float angleInDegrees) {
  ...
  float offsetX =
      currentPosition.x -
      cos(currentAngle) * (currentPosition.x - m_body->GetWorldCenter().x) +
      sin(currentAngle) * (currentPosition.y - m_body->GetWorldCenter().y);
  float offsetY =
      currentPosition.y -
      sin(currentAngle) * (currentPosition.x - m_body->GetWorldCenter().x) -
...
float Vehicle::GetAngle() { return m_body->GetAngle(); }
...
void Vehicle::UpdateLateralVelocity() {
  ...
  b2Vec2 lateralVelocity =
      b2Dot(currentRightNormal, m_body->GetLinearVelocity()) *
      currentRightNormal;
...
void Vehicle::CrazyRotate(float degree, float intensity) {
  ...
  b2Vec2 linearForce =
      b2Vec2(cos(m_body->GetAngle() + radians) * intensity * m_body->GetMass(),
  ...
  float torque =
      intensity * m_body->GetMass();  // Adjust the multiplier as needed
...
void Vehicle::ApplyBuff(float forceMul, float MaxSpeedMul, float SizeMul,
                        float TorqueMul) {
  forceBuff *= forceMul;
...
void Vehicle::UpdateCoolDown() {
  if (superSkillCoolDown > 0) superSkillCoolDown--;
};
...
void Vehicle::SuperSkill() { std::cout << "HI" << std::endl; }

void Vehicle::MagneticPull(float radius) {
  b2Fixture* fixture = m_body->GetFixtureList();
  if (fixture) {
    b2Shape* shape = fixture->GetShape();
    dynamic_cast<b2PolygonShape*>(shape)->SetAsBox(BOX_WIDTH * radius,
                                                   BOX_HEIGHT * radius);
  }
}

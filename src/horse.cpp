...
HorseSuperSkillBuff::HorseSuperSkillBuff(int duration, float Intensity)
...
Horse::Horse(b2World* world, float x /*= 0*/, float y /*= 0*/,
...
void Horse::SuperSkill() {
  if (superSkillCoolDown == 0) {
    HorseSuperSkillBuff* buff =
        new HorseSuperSkillBuff(HORSE_SKILL_DURATION, HORSE_SKILL_INTENSITY);
    AddBuff(buff);
    superSkillCoolDown = HORSE_SKILL_COOLDOWN;
  }
}
...
void ResourceManager::LoadImage(const std::string& key,
                                const std::string& filename) {
  if (textures_.find(key) != textures_.end()) {
    std::cerr << "Warning: The texture " << key << " is already loaded"
              << std::endl;
    return;
  }
...
const sf::Texture& ResourceManager::GetImage(const std::string& key) const {
  ...
  if (it != textures_.end()) {
    ...
    std::cerr << "Texture not found: " << key << ". Using default texture."
              << std::endl;
...
void ResourceManager::LoadFont(const std::string& key,
  ...
  if (fonts_.find(key) != fonts_.end()) {
    std::cerr << "Warning: The font " << key << " is already loaded"
              << std::endl;
    return;
  }
...
const sf::Font& ResourceManager::GetFont(const std::string& key) const {
  ...
  if (it != fonts_.end()) {
    ...
    std::cerr << "Font not found: " << key << ". Using default font."
              << std::endl;
    // Return default font or handle error
  }
}

void ResourceManager::LoadSoundBackground(const std::string& key,
                                          const std::string& filename) {
  if (sounds_background.find(key) != sounds_background.end()) {
    std::cerr << "Warning: The sound " << key << " is already loaded"
              << std::endl;
    return;
  }
...
const sf::SoundBuffer& ResourceManager::GetSoundBackground(
  ...
  if (it != sounds_background.end()) {
    ...
    std::cerr << "Sound not found: " << key << ". Using default sound."
              << std::endl;
    // Return default font or handle error
  }
}

void ResourceManager::LoadSoundStep(const std::string& key,
                                    const std::string& filename) {
